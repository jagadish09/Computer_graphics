			...
			if (j%resolution == 0) {
				if (i%resolution == 0) {
					...
					if (projection == 0)
					{
						...
						color = objBuffer.CastRay(cam.GetEye(), c, depth, objBuffer,0); //finding hit point and shading for the pixel
					}
					...
					else//setting up orthographic projection
					{
						...
						Vector c1;
						c1.set(0, 0, -1);
						color = objBuffer.CastRay(s1, c1, depth, objBuffer,0);
					}
				}
				else {
...
void setRayTracingMaterials() {
	...
	objBuffer.GetObj(sphere1)->obj.rindex = 1.23;//refractive index of the sphere3
	...
	objBuffer.GetObj(sphere2)->obj.rindex = 1.12;//refractive index of the sphere2
	...
	objBuffer.GetObj(sphere3)->obj.rindex = 0.4;//refractive index of the sphere3
}
