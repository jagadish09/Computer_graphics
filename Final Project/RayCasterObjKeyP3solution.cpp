...
Color RayCasterObjBuffer::CastRay(Point& s, Vector& c, int d, RayCasterObjBuffer objBuffer,int loc) {//added location which tells the ray is inside object or outside
	RayCasterObjNode* cur;
	...
	for (i = 1; i <= count; i++) {
		...
		if (!cur->obj.hide) {
			...
			if (((hit > 0.0f) && ((hit < closestHit) || (closestHit <= 0)))) {
				...
				if (loc == 0)
				{
					r = (float)(1.0 /(float) closest->obj.rindex);
					a = asin(closest->obj.rindex)*180.0/PI;//calculating critical angle if the ray is hitting object from outside
				}
				else if (loc == 1)
				{
					r = closest->obj.rindex;
					a = asin((float)(1 / ((float)closest->obj.rindex)))*180.0/PI;//calculating critical angle if the ray is hitting object from inside
				}
				...
				float e = acos(((float)(c.dot(nvector))) / ((float)(c.magnitude()*nvector.magnitude())))*180.0 / PI;
				float angle=180-e;//angle of incidence
				if(angle<a)//checking if incident angle is less than critical angle
				{//implementing refraction
					if (loc == 0)
						nextloc = 1;//changing location of the ray from outside to inside
					else
						nextloc = 0; //changing location of the ray from inside to outside
					Vector w = nvector * (-r*q - sqrt(1 - pow(r, 2)*(1 - pow((q), 2))));
					rvector = (c*r) + w;//refraction vector
					refract = 1;

				}
				
				else//if incident angle is greater than critical angle i.e total internal reflection
				{
					rvector = c - nvector*q * 2;//calculating rflected vector
					nextloc = loc;//next location of the ray doesnt change . i.e if the ray is inside then it's next location going to be inside of the object.
				}
				...
				else
				{
					...
					if (d == 0 || d == 1 || (shadows == true && isInShadow(hitpoint, c, objBuffer) == 1))//checking for depth and shadows
						return background;
					recolor = CastRay(hitpoint, rvector, d - 1, objBuffer,nextloc);//recursively calling cast ray for reflection for depth d
					if (loc == 1 && nextloc == 1)//if its the ray has complete internal reflection with in the object.
					{
						col = closest->obj.objColor;
					}
					else
